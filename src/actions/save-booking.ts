...
export async function saveBooking(data: BookingFormInput) {
  ...
  try {
    ...
    // Send email to user (server-side)
    await sendEmail("template_0f6ylcp", userEmailParams, data.email);

    console.log("Admin email parameters:", adminEmailParams); // Log admin email parameters

    // Send email to admin (server-side)
    await sendEmail("template_lsv899c", adminEmailParams, "chicoynano1@gmail.com"); // Admin email

    return {
      success: true,
      message: "Reserva guardada correctamente. Se han enviado los correos de confirmación."
    };
  } catch (error: any) {
